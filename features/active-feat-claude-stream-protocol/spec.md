...
### General Checklist
- [ ] Thread 模式和 Channel 模式均支持新参数
- [ ] `--input-format stream-json` 正确启用 stdin JSON 协议
- [ ] `--permission-mode bypassPermissions` 全自动执行
- [ ] `control_response` 自动批准 fallback 正常工作
- [ ] `--strict-mcp-config` 注入受控 MCP 配置
- [ ] MCP config 临时文件在进程结束后清理
- [ ] 不影响现有 A2A adapter 的 Claude Code 通信
- [ ] 向后兼容：不破坏未配置 MCP 的 Agent
